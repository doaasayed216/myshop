        ...
        <div class="flex flex-col w-full px-0 mx-auto md:flex-row">
            <div class="flex flex-col md:w-full float-left">
                ...
                <div class="border border-black w-3/4 p-5 mb-5">
                    <div class="float-left">
                        <h2 class="text-l font-semibold">Shipping address <a href="/address" class="text-blue-500">Change</a></h2>
                        <p>{{auth()->user()->name}}</p>
                        <p>{{$address->details}}</p>
                        <p>{{$address->phone}}</p>
                    </div>
                    ...
                    <div class="float-right">
                        <h2 class="text-l font-semibold">Payment method <a href="/payment" class="text-blue-500">Change</a></h2>
                        <p>Cash On Delivery(COD)</p>
                    </div>
                </div>

                <div class="border border-black w-3/4 p-5">
                    <p class="text-xl font-semibold mb-5 text-green-500">Delivery: {{now()->addDay(2)}}
                        <span class="font-semibold hover:text-red-500 text-gray-500 text-xs">
                            If you order in the next 13 hours and 41 minutes
                        </span>
                    </p>
                    <div class="float-left">
                        @foreach(auth()->user()->cart->products as $product)
                            <div class="flex mb-3"> <!-- product -->
                                ...
                                <div class="flex flex-col justify-between ml-4 flex-grow">
                                    ...
                                    <p class="font-semibold hover:text-red-500 text-gray-500 text-xs">{{'Sold by: '. 'seller'}}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="float-right border border-black w-1/4 mt-10 p-5 h-64 mr-5">
                <form method="post" action="/place/order">
                    @csrf
                    <button class="border rounded-xl bg-yellow-500 p-3 text-white text-center w-full mb-5">Place Your Order</button>
                </form>
                <div>
